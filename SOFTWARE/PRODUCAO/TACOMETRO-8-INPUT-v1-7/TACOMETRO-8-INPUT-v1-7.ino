...
|                multiplexação de 3 portas digitais         |
|                                                           |
 ----------------------------------------------------------
...
// Define endereço do micro baseado nos pinos de entrada uma tabela binaria orienta a config
void Gerenciador_endereco   (                                                           );
// Contador de pulsos das entradas dos sensores.
int  Contador               (                                                           );
// Monta o vetor com pulsos convertidas em RPM simples, sem média movel dado bruto
void Vetor_RPM              (float ref_temp,int n_pulso,int n_pulso_volta,int end_sensor);
// abre e fecha a rede 485 função herdada termometria
void Proto_485_V2           (String end_cliente, String msg_cliente                     );
// temperatura convertida enviada via rede 
...
void setup() {
   ...
   Serial.begin(9600);
   Serial.println("Arquivo:TACOMETRO-8-INPUT-v1-7...\n");
...
// PROTOCOLO DE REDE INTERNA ATUAL
    /* O cliente 1  envia via sua leitura via rede e depois
        muda o estado das portas para o cliente numero 2 responder espera o timeout
        depois muda estado das portas para o cliente 3... até voltar ele de novo. */
// FUNCIONAMENTO DO PRODUTO
    /* O programa lê o endereço do cliente configurado por meio de dip switchs com 3 pontos
       representadano uma sequencia binaria b0b1b2 onde teremos as combinações:
       cliente 1        001 
       cliente 3        011 
       como exemplo das possíveis combinações
       depois dessa configuração inicial o produto le as portas destinadas a demanda de rede
       nessa etapa ele compara se recebeu comando externo para responder uma solicitação
       via rede 485 ou se segue para a rotina de leitura e processamento dos pulsos dos sensores
       a janela de intervalo de respostas para uma solicitação de rede é de 1 segundo */
void loop() {
  wdt_reset               (     );
